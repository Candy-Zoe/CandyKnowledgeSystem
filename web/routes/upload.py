...
def upload_page():
    ...
    return render_template("upload.html", supported_types=DocumentParser.SUPPORTED_TYPES)
...
def upload_files():
    ...
    for file in files:
        ...
        if file_type not in SUPPORTED_TYPES:
            results.append({"filename": original_name, "error": f"不支持的文件类型: .{file_type}"})
            continue
        ...
        metadata = DocumentParser.get_metadata(file_path, file_type)
    ...
    return jsonify({"message": f"已上传 {len(results)} 个文件", "results": results})
